...
start = (25, 350)  # Starting position
goal = (300, 120)   # Goal position
maze_height = 390
maze_width = 450
...
def transform_and_convert_image_to_matrix(image_path, transforming=True,
    ...
    if transforming :
        ...
        if ids is not None and len(ids) == 4: # check marker are detcted or not, if yes:
            ...
            perspective_matrix = cv2.getPerspectiveTransform(src_points, destionation_points) # compute perspective transformation matrix
            transformed_image = cv2.warpPerspective(image, perspective_matrix, (width, height))  # apply it
            cv2.imwrite(transformed_image_filename, transformed_image) #save_transformed image
...
def clean_up_path(moves, start=start,  move_size_limit=15):
...
def is_within_distance_to_goal(x1, y1, x2, y2, threshold):
    return True if (x1 - x2) ** 2 + (y1 - y2) ** 2 < threshold else False
