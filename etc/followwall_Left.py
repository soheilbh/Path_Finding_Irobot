...
robot = Create3(Bluetooth('Robot 5'))

speed = 15.0

Distance_Left_far = 90
Distance_Left_close = 200
Distance_Right_close = 1200
Distance_Front = 90

@event(robot.when_bumped, [False, True]) # Right Bumped
async def bumped_Right(robot):
    await robot.move(-10)
    await robot.turn_left(45)

@event(robot.when_bumped, [True, False]) # Left Bumped
...
async def test(robot):
    ...
    while True:
        ...
        if SensorRight >= Distance_Right_close: # Go Left
            print("Close Right")
